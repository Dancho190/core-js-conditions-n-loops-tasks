 ...
 */
/**
 * Sorts an array of numbers in ascending order in place using Quick Sort.
 *
 * @param {number[]} arr - The array to sort.
 * @return {number[]} - The sorted array.
 */
  ...
  function merge(start, mid, end) {
    ...
    while (leftIndex <= mid && rightIndex <= end) {
      if (temp[leftIndex] <= temp[rightIndex]) {
        sortedArray[currentIndex] = temp[leftIndex];
        leftIndex += 1;
      } else {
        sortedArray[currentIndex] = temp[rightIndex];
        rightIndex += 1;
      }
      currentIndex += 1;
    }
...
function findCycleLength(str) {
  ...
  while (!seen.has(currentStr)) {
    ...
    for (let index = 0; index < length; index += 1) {
      if (index % 2 === 0) {
        evenIndexStr += currentStr[index];
      } else {
        oddIndexStr += currentStr[index];
      }
    }

    currentStr = evenIndexStr + oddIndexStr;
